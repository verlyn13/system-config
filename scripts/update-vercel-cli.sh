...
current_version=$(vercel --version 2>/dev/null | grep -o '[0-9]\+\.[0-9]\+\.[0-9]\+' | head -1)
latest_version=$(npm view vercel version 2>/dev/null)
...
if npm update -g vercel; then
    new_version=$(vercel --version 2>/dev/null | grep -o '[0-9]\+\.[0-9]\+\.[0-9]\+' | head -1)
