...
if ! command -v copilot >/dev/null 2>&1; then
    log_error "GitHub Copilot CLI not found. Installing..."
    npm install -g @github/copilot
    exit $?
fi

# Get current and latest versions
current_version=$(copilot --version 2>/dev/null | grep -o '[0-9]\+\.[0-9]\+\.[0-9]\+' | head -1)
...
if npm update -g @github/copilot; then
    new_version=$(copilot --version 2>/dev/null | grep -o '[0-9]\+\.[0-9]\+\.[0-9]\+' | head -1)
