# desktop-2jj3187-ssh-lane-install-v0.5.0.ps1
#
# Phase 3 install of the DESKTOP-2JJ3187 SSH admin lane.
#
# v0.5.0 supersedes v0.4.0 after the v0.4.0 attempt halted at S7.
# Root cause: Microsoft's default sshd_config does NOT contain an
# Include directive, so the drop-in file at
# C:\ProgramData\ssh\sshd_config.d\20-jefahnierocks-admin.conf was
# silently ignored by sshd. See:
#   docs/device-admin/desktop-2jj3187-ssh-lane-install-v0.4.0-incident-2026-05-15.md
#   docs/device-admin/windows-terminal-admin-spec.md (Windows OpenSSH Defaults)
#
# v0.5.0 adds a new S3b step that injects
#   Include sshd_config.d/*.conf
# at the top of sshd_config (idempotent, snapshot-backed). All other
# steps are identical to v0.4.0. v0.4.0's S1-S6 mutations are
# idempotent; re-running v0.5.0 on the v0.4.0 partial-apply host
# state reads S1-S6 as already-done and only S3b + the S7 restart
# and readback do new work.
#
...
#
# Prerequisite: either
#   (a) docs/device-admin/desktop-2jj3187-reconciliation-2026-05-15.md
#       applied and its apply record committed (fresh-host case), or
#   (b) docs/device-admin/desktop-2jj3187-ssh-lane-install-v0.4.0-2026-05-15.md
#       attempted, halted at S7 with partial apply through S6
#       (v0.4.0 partial-apply case).
#
# Surfaces mutated by this script:
#   - OpenSSH.Server Windows capability (install)
#   - sshd service (StartType=Automatic, Status=Running)
#   - C:\ProgramData\ssh\sshd_config (Match Group administrators block;
#     Include sshd_config.d/*.conf at top)
...
# Runbook: docs/device-admin/desktop-2jj3187-ssh-lane-install-v0.5.0-2026-05-15.md
...
# ==================================================================
# S3b -- Inject Include sshd_config.d/*.conf at top of sshd_config
# ==================================================================
# Microsoft's default sshd_config does NOT contain an Include
# directive. Without it, files placed in C:\ProgramData\ssh\sshd_config.d\
# are silently ignored by sshd. v0.4.0 missed this and halted at S7.
# This step injects the Include directive at the top of sshd_config,
# above any Match block (Include must precede Match blocks so that
# the directives loaded by Include apply in the global scope).
Step-Log 'S3b' 'ensure Include sshd_config.d/*.conf at top of sshd_config'

$includeLine  = 'Include sshd_config.d/*.conf'
$includeRegex = '^\s*Include\s+sshd_config\.d/\*\.conf\s*$'

$cfgLines       = Get-Content -LiteralPath $sshdConfigPath
$hasIncludeLine = @($cfgLines | Where-Object { $_ -match $includeRegex }).Count -gt 0

$wroteInclude = $false
if ($hasIncludeLine) {
    Step-Log 'S3b' "Include directive already present at top of $sshdConfigPath; skip"
} else {
    # Snapshot the post-S3 / pre-S3b state explicitly. (S3's
    # sshd_config.preinstall is the pre-S3 snapshot; if S3 was a no-op
    # the two are identical, but capture a distinct rollback marker
    # for this step's audit trail.)
    Copy-Item -LiteralPath $sshdConfigPath `
        -Destination (Join-Path $SnapDir 'sshd_config.pre-include-inject') -Force

    # Prepend Include line plus a blank separator, preserving all
    # other content verbatim.
    $newLines = @($includeLine, '') + $cfgLines
    Set-Content -LiteralPath $sshdConfigPath -Value $newLines -Encoding ascii
    $wroteInclude = $true
    Step-Log 'S3b' 'Include sshd_config.d/*.conf injected at top of sshd_config'
}

if (-not (Test-SshdConfig)) {
    Step-Log 'S3b' 'sshd -t failed after S3b; restoring snapshot.'
    $rollbackFrom = if (Test-Path -LiteralPath (Join-Path $SnapDir 'sshd_config.pre-include-inject')) {
        Join-Path $SnapDir 'sshd_config.pre-include-inject'
    } else {
        Join-Path $SnapDir 'sshd_config.preinstall'
    }
    Copy-Item -LiteralPath $rollbackFrom -Destination $sshdConfigPath -Force
    throw 'sshd -t failed after Include-directive injection.'
}

# Read-back: confirm the line is present after the edit.
$cfgLinesAfter      = Get-Content -LiteralPath $sshdConfigPath
$includePresentAfter = @($cfgLinesAfter | Where-Object { $_ -match $includeRegex }).Count -gt 0

if (-not $includePresentAfter) {
    throw 'Include directive readback failed: Set-Content reported success but no matching line found in sshd_config.'
}

Write-Json -FileName '03b-include-directive.json' -Value ([pscustomobject]@{
    sshd_config_path  = $sshdConfigPath
    include_line      = $includeLine
    include_present   = $includePresentAfter
    wrote_in_this_run = $wroteInclude
})
...
$dropInBody = @"
# Managed by system-config docs/device-admin/desktop-2jj3187-ssh-lane-install-v0.5.0-2026-05-15.md
...
# administrators_authorized_keys and that the drop-in directives
# loaded via Include sshd_config.d/*.conf are in effect.
