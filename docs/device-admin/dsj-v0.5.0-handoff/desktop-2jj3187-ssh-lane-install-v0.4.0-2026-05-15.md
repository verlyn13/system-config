---
title: DESKTOP-2JJ3187 SSH Lane Install Packet v0.4.0 - 2026-05-15 (SUPERSEDED)
category: operations
...
# DESKTOP-2JJ3187 SSH Lane Install Packet v0.4.0 - 2026-05-15 (SUPERSEDED)
...
>
> v0.4.0 applied S1-S6 cleanly on DESKTOP-2JJ3187 and halted at S7
> when the effective-config readback (`sshd -T`) did not contain
> the directives the drop-in file declared. Root cause: Microsoft's
> default `sshd_config` does **not** include any `Include` directive
> — the drop-in at `C:\ProgramData\ssh\sshd_config.d\` was silently
> ignored. v0.4.0 was designed against the Linux-distro convention
> (OpenSSH 8.2+ ships with `Include sshd_config.d/*.conf` near the
> top) and did not verify against Microsoft's upstream
> `sshd_config`.
>
> v0.5.0 adds a new **S3b** step that injects
> `Include sshd_config.d/*.conf` at the top of `sshd_config`
> (idempotent, snapshot-backed). All other steps are unchanged;
> v0.4.0's idempotent gates mean re-running v0.5.0 against the
> partial-apply host state reads S1-S6 as "already done" and only
> S3b + the S7 restart-and-readback do new work.
>
> See:
...
>   — full postmortem with the host-state inventory
> - [windows-terminal-admin-spec.md](./windows-terminal-admin-spec.md)
>   `§Windows OpenSSH Defaults` — the new spec section that should
>   have existed before v0.4.0 shipped
>
> **Preserve everything in place**: do not delete or rotate the
> v0.4.0 evidence directory on the host
> (`C:\Users\Public\Documents\jefahnierocks-device-admin\desktop-2jj3187-ssh-install-<UTC-timestamp>\`).
> Do not edit or re-encode this Markdown file. Per
> [§Packet-Defect Halt Rule](./windows-terminal-admin-spec.md),
> the failed artifacts stay verbatim as the postmortem trail.
