...
| MAMAWORK mini-PC | [mamawork-homenetops-lan-identity-2026-05-14.md](./mamawork-homenetops-lan-identity-2026-05-14.md) | Ingest of the HomeNetOps PASS hand-back for MAMAWORK. OPNsense static DHCP reservation bound, Unbound override `mamawork.home.arpa -> 192.168.0.101`, LAN/igc1 ARP confirms IP-to-MAC, `dig` PASS from LAN resolver. TCP/22 from fedora-top times out; HomeNetOps explicitly notes this is a Windows-side issue (next packet target). ARP `permanent=false` because MAMAWORK is host-static; switching to DHCP would activate ISC static-ARP defense (optional follow-up packet). |
...
| MAMAWORK mini-PC | [mamawork-ssh-investigation-packet-2026-05-14.md](./mamawork-ssh-investigation-packet-2026-05-14.md) | Prepared. Read-only Windows-side investigation of why TCP/22 from `fedora-top` to `mamawork.home.arpa` times out despite HomeNetOps PASS. From an elevated PowerShell session, captures network category, sshd service state, port-22 listener binding, every inbound TCP/22 firewall rule (including the `Dad Remote Management` rule), sshd_config directives that could refuse the LAN client, authorized_keys posture, and OpenSSH/Operational + Security 4625 event log entries. Writes output to `C:\Users\Public\Documents\jefahnierocks-device-admin\mamawork-ssh-investigation-<ts>\`. Does NOT start/stop/edit any service or rule. |
...
| Fleet (Cloudflare) | [cloudflare-dns-handback-ingest-2026-05-14.md](./cloudflare-dns-handback-ingest-2026-05-14.md) | Authoritative `cloudflare-dns` handback (cited commit `b5b9460`, path `docs/handback-system-config-2026-05-13.md` in the cloudflare-dns repo). Single Cloudflare account, team `homezerotrust`, IaC is Pulumi-TypeScript (NOT OpenTofu), tokens in **gopass** (NOT 1Password). WARP active on 7 fleet devices but neither fedora-top nor MAMAWORK is enrolled yet. Identity providers: Google OAuth + email OTP. Four profiles (Default + Kids locked + Adults unlocked + Headless MDM). Profile recommendations: **Kids profile** for both fedora-top (Wyn user, WARP identity `wynrjohnson@gmail.com`) and MAMAWORK (with explicit Litecky trade-off; pick `axelptjohnson@gmail.com` or `wynrjohnson@gmail.com` as the WARP identity). WARP is system-level: one identity per machine. No SSH Access app or Tunnel exists today (working naming candidates recorded). Supersedes any earlier mistakenly-circulated "cloudflare-dns reply" text. |
...
| MAMAWORK mini-PC | [mamawork-ssh-key-bootstrap-packet-2026-05-14.md](./mamawork-ssh-key-bootstrap-packet-2026-05-14.md) | Prepared. Operator-applied on MAMAWORK via elevated PowerShell. Installs the new 1Password-backed admin key (`SHA256:qilvkR7/539qqRoWurVdAgoXL1Wol7WzbD0tHlha0QY`, item `op://Dev/jefahnierocks-device-mamawork-admin-ssh-verlyn13`) into `C:\ProgramData\ssh\administrators_authorized_keys` and mirrors into `C:\Users\DadAdmin\.ssh\authorized_keys`, gated by fingerprint match. Legacy `DadAdmin_WinNet` line left in place (private half confirmed missing on fedora-top; dangling but harmless; cleanup deferred to a follow-up). Remote verification deferred to after the MAMAWORK SSH investigation packet remediates TCP/22. |
| MAMAWORK mini-PC | [mamawork-lan-rdp-implementation-2026-05-14.md](./mamawork-lan-rdp-implementation-2026-05-14.md) | Prepared. Operator-applied on MAMAWORK via elevated PowerShell 7+. Mirrors the DESKTOP-2JJ3187 RDP pattern: NetworkCategory Private; `fDenyTSConnections=0`; `UserAuthentication=1` (NLA); TermService Running+Automatic; disable built-in Remote Desktop firewall group; create custom `Jefahnierocks RDP LAN TCP 3389` + `UDP 3389` (Inbound Allow, Private, RemoteAddress `192.168.0.0/24`); hibernate off + AC sleep/hibernate 0 + hybrid sleep 0; idempotent NIC wake properties; evidence to `C:\Users\Public\Documents\jefahnierocks-device-admin\mamawork-rdp-lan-<ts>.txt`. Does NOT touch SSH/sshd/authorized_keys/firewall-22, WinRM, accounts, BitLocker, Tailscale, Cloudflare, WARP, cloudflared, OPNsense, DNS, DHCP, 1Password, or the host-static-vs-DHCP question. |
...
- Treat Fedora LAN SSH as established and hardened, privilege cleanup
  ...
  HomeNetOps outbound is answered as of 2026-05-14 (MAMAWORK PASS);
  the Hetzner repo ingested as advisory only; the **cloudflare-dns
  outbound is now answered** as of 2026-05-14 (citing cloudflare-dns
  commit `b5b9460`). The current packet order is recorded in
  `current-status.yaml.recommended_packet_order_2026_05_14`: A and B
  are done (HomeNetOps + Hetzner ingest, Fedora admin backup packet
  prepared); C and D are done (MAMAWORK SSH investigation packet and
  the optional DHCP source-of-truth packet prepared). The next live
  actions in priority order are: (1) apply the Fedora admin-backup
  SSH key strategy packet to close the single-MacBook dependency,
  (2) apply the MAMAWORK SSH investigation packet read-only to
  identify the TCP/22 cause, (3) optionally apply the MAMAWORK DHCP
  source-of-truth packet on a separate window, (4) keep the
  fedora-top known_hosts reconciliation available as a small
  cleanup. E (WARP/cloudflared cutover) stays blocked on the actual
  cloudflare-dns handback; do not let the missing handback delay
  steps 1-4. The remaining approval-gated items remain the WARP /
  Cloudflare off-LAN cutover, the optional Tailscale login-with-ACL
  break-glass packet, LUKS/power policy, a future narrow review of
