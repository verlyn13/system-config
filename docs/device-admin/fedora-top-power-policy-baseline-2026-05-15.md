...
- **v0.4.0 (2026-05-15)**: Fix diagnostic fallback semantics for
  `HandleLidSwitchExternalPower`. v0.3.0 captured the correct raw
  values but its case statement only matched the literal value, so
  an empty value (the systemd default, and the actual cause of the
  2026-05-14 19:23 suspend incident) wrongly reported
  `diagnostic.lid_close_on_ac_will_suspend = false`. logind.conf(5)
  is explicit: when `HandleLidSwitchExternalPower` is empty,
  behavior falls through to `HandleLidSwitch`. v0.4.0 resolves the
  empty value to `HandleLidSwitch` before evaluating the case, so
  the diagnostic matches observed behavior. Other diagnostic
  booleans (`lid_close_on_battery_will_suspend`, `idle_will_suspend`)
  use direct properties with no fallback semantic and are unchanged.
  This is a pure annotation fix; the apply script does not gate on
  baseline diagnostics, so v0.2.0 of apply is unaffected.
...
- **v0.2.0 (2026-05-15)**: Fix step 10 ARG_MAX overflow. v0.1.0 failed
  ...
  `fedora-top-power-policy-baseline-v0.4.0.sh: line 47: /usr/bin/jq: Argument list too long`.
...
sha256:     b0c411b06eae09a616a25ba1d65eba3f91e6a014ee4a3bc102c693b78feea2ae
encoding:   ASCII (python: 20449 bytes, 0 bytes > 0x7F; 500 lines)
...
scp scripts/device-admin/fedora-top-power-policy-baseline-v0.4.0.sh \
    fedora-top:/var/tmp/
...
```bash
# From the MacBook:
ssh fedora-top 'bash -s' < scripts/device-admin/fedora-top-power-policy-baseline-v0.4.0.sh
```
...
> Run the `fedora-top-power-policy-baseline-v0.4.0` script on
