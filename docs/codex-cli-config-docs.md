...
Control whether Codex can issue web searches with the `web_search` setting (top-level or per profile):

```toml
web_search = "live" # or "cached", "disabled"
```

`web_search_request` is deprecated; migrate to `web_search` to avoid warnings.
