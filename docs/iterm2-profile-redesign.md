...
1. **Project-path independence** — profiles must not bake `~/Organizations/...` into JSON. Project moves should require zero profile edits.
...
| `_iterm2_emit_badge` precmd hook | No | Pure `printf` + `base64` of an env var. Local-only. |
