...
| `github/happy-patterns/macpro-ssh-key` | `op://Dev/ssh-github-happy-patterns` | SSH key item | 1Password SSH agent; Git authentication and SSH signing for the `happy-patterns` identity; public key rendered by `home/private_dot_ssh/id_ed25519_happy_patterns.1password.pub.tmpl` | Happy Patterns | `happy-patterns` GitHub account | Event-driven rotation on compromise, custody change, or identity boundary change | `active` | `docs/ssh.md`; `docs/secrets.md` | Stop if reused as an unattended machine identity, if provider account ownership changes, or if project automation depends on this human-interactive key. |
...
| 2026-05-09 | `github/jefahnierocks/macpro-mcp` | runtime containment changed from soft empty-alias failure to hard disable marker | added `~/.local/state/system-config/mcp-github.disabled`; wrapper exits `78` before env/op token resolution | live `~/.local/bin/mcp-github-server` exits `78`; wired alias remains blocked pending no-argv bridge |
